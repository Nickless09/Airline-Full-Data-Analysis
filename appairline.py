...
# source_col = get_col(df, "source city", "source", "from")
# dest_col = get_col(df, "destination city", "destination", "to")
# class_col = get_col(df, "class", "travel class")
# price_col = get_col(df, "price", "fare", "ticket price")
# duration_col = get_col(df, "duration", "flight time", "time taken", "time")
# days_left_col = get_col(df, "days left", "days before", "daysuntilflight")
...
# # --- KPIs ---
# avg_price = round(filtered_df[price_col].mean(), 2) if price_col else 0
# avg_duration = round(filtered_df[duration_col].mean(), 2) if duration_col else 0
# total_flights = len(filtered_df)
# min_price = round(filtered_df[price_col].min(), 2) if price_col and not filtered_df.empty else 0
# max_price = round(filtered_df[price_col].max(), 2) if price_col and not filtered_df.empty else 0
...
# col4.metric("Cheapest Flight (₹)", min_price)
# col5.metric("Most Expensive Flight (₹)", max_price)

# # --- Charts ---
# if price_col:
#     st.subheader("Average Price by Airline")
#     fig1 = px.bar(
#         filtered_df.groupby(airline_col)[price_col].mean().reset_index(),
#         x=airline_col, y=price_col, color=airline_col,
#         title="Average Price per Airline"
#     )
#     st.plotly_chart(fig1, use_container_width=True)

# if price_col and days_left_col:
#     st.subheader("Price vs. Days Left")
#     fig2 = px.scatter(
#         filtered_df, x=days_left_col, y=price_col, color=airline_col,
#         trendline="ols", title="Price vs Days Left by Airline"
#     )
#     st.plotly_chart(fig2, use_container_width=True)

# if price_col and duration_col:
#     st.subheader("Flight Duration vs. Price")
#     fig3 = px.scatter(
#         filtered_df, x=duration_col, y=price_col, color=class_col if class_col else None,
#         title="Flight Duration vs Price" + (" (by Class)" if class_col else "")
#     )
#     st.plotly_chart(fig3, use_container_width=True)

# if price_col:
#     st.subheader("Average Price by Source-Destination Route")
#     route_df = filtered_df.groupby([source_col, dest_col])[price_col].mean().reset_index()
#     fig4 = px.bar(
#         route_df, x=source_col, y=price_col, color=dest_col,
#         title="Average Price by Route", barmode="group"
#     )
