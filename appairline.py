...
for file in [CLEAN_FILE, ECONOMY_FILE, BUSINESS_FILE]:
...
st.subheader("Number of Flights per Route")

route_count_df = (
    filtered_df.groupby([source_col, dest_col])
    .size()
...
if not route_count_df.empty:
    fig_route = px.density_heatmap(
        ...
        title="Flight Counts by Source-Destination Route",
        labels={source_col: "Source City", dest_col: "Destination City", "flight_count": "Number of Flights"}
    )
